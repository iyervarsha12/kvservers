...
type config struct {
	...
	n             int
	kvservers     []*KVServer
	saved         []omnipaxoslib.Persistable
...
func (cfg *config) StartServer(i int) {
	...
	if cfg.saved[i] != nil {
		...
		cfg.saved[i] = omnipaxos.MakePersister()
	}
	cfg.mu.Unlock()

	cfg.kvservers[i] = StartKVServer(ends, i, cfg.saved[i], cfg.maxpaxosstate)
...
func make_config(t *testing.T, n int, unreliable bool, maxpaxosstate int) *config {
	ncpu_once.Do(func() {
		if runtime.NumCPU() < 2 {
	...
	cfg.saved = make([]omnipaxoslib.Persistable, cfg.n)
	cfg.endnames = make([][]string, cfg.n)
	cfg.clerks = make(map[*Clerk][]string)
	cfg.nextClientId = cfg.n + 1000 // client ids start 1000 above the highest serverid
	cfg.maxpaxosstate = maxpaxosstate
	cfg.start = time.Now()
...
func (cfg *config) end() {
	...
	if cfg.t.Failed() == false {
		...
		npeers := cfg.n                    // number of OmniPaxos peers
